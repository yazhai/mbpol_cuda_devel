...
//			propogation through the network begins). Transposing Bias is trivial.
//***********************************************************************************
...
//class for Network of Layers(Collection of Layer Structs that can 
//implement movement of input through network)
template<typename T>
class Layer_Net_t{
private: 
	...
	//Move through network and make prediction based on all layers.
     void predict(T* _inputData, int _N, int _input, T* & _outputData, unsigned long int& _outsize){
		if (root != NULL) {
			...
			//this line fails for some reason
...
// tester function, including reading HDF5 file, creating layers, and making the prediction.
//This one is for the new test, using datafile: "16.hdf5" 
template <typename T>
	...
	T* finalOutput = nullptr;    
     ...
     try{     
		...
		for(int ii=0;ii<numAtoms;ii++){
			...
			cout<<"finalOutput[1] = "<<finalOutput[1]<<endl;
			
			//final scores to file
			outputFile.open(filePath,ofstream::out|ofstream::app);
			outputFile<<" NEXT ATOM"<<endl;
